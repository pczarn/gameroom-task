  ...
  <form @submit.prevent="prepareAndSubmit" v-if="match && match.teamOne && match.teamTwo">
    ...
    <fieldset class="row">
      <select @input="setTeamOne" class="left-col">
        <option :selected="!teamOneId" value> -- select a team -- </option>
        <option v-for="team in potentialTeamOne" :value="team.id" :selected="team.id === teamOneId">
          {{ team.name }}
        </option>
      </select>
      <span class="middle-col">vs</span>
      <select @input="setTeamTwo" class="right-col">
        <option selected="!teamTwoId" value> -- select a team -- </option>
        <option v-for="team in potentialTeamTwo" :value="team.id" :selected="team.id === teamTwoId">
          {{ team.name }}
        </option>
      </select>
    </fieldset>
    ...
    <fieldset class="row" :class="{ err: !!formErrors.members }">
      <div class="left-col" :class="{ err: !!formErrors.team_one }">
        <ul>
      ...
      <span class="middle-col"></span>
      <div class="right-col" :class="{ err: !!formErrors.team_two }">
        <ul>
...
export default {
  ...
  computed: {
    ...
    potentialTeamOne () {
      const excluded = new Set(this.match.teamTwo.members.map(m => m.id))
      return this.teamList.filter(team => team.members.every(member => !excluded.has(member.id)))
    },
    potentialTeamTwo () {
      const excluded = new Set(this.match.teamOne.members.map(m => m.id))
      return this.teamList.filter(team => team.members.every(member => !excluded.has(member.id)))
    },
    playedAtTime () {
